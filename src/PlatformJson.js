...
class PlatformJson {
    ...
    /**
     * Indicates whether the specified plugin is installed as a top-level (not as dependency to others)
     *
    ...
    /**
     * Indicates whether the specified plugin is installed as a dependency to other plugin.
     *
     * @param  {String} pluginId A plugin id to check for.
     *
    ...
    isPluginInstalled (pluginId) {
        return this.isPluginTopLevel(pluginId) || this.isPluginDependent(pluginId);
    }

    addPlugin (pluginId, variables, isTopLevel) {
        const pluginsList = isTopLevel ? this.root.installed_plugins : this.root.dependent_plugins;
    ...
    removePlugin (pluginId, isTopLevel) {
        var pluginsList = isTopLevel ? this.root.installed_plugins : this.root.dependent_plugins;
    ...
    removePluginMetadata (pluginInfo) {
        ...
        this.root.modules = installedModules
            .filter(installedModule => !modulesToRemove.includes(installedModule.file));
...
class ModuleMetadata {
    /**
     * Creates a ModuleMetadata object that represents module entry in 'cordova_plugins.js' file at run time
     *
