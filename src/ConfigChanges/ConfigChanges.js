...
class PlatformMunger {
    ...
    add_plugin_changes (pluginInfo, plugin_vars, is_top_level, should_increment, plugin_force) {
        ...
        if (!edit_config_changes || edit_config_changes.length === 0) {
            ...
            const isConflictingInfo = is_conflicting(edit_config_changes, platform_config.config_munge, this, plugin_force);
    ...
    add_config_changes (config, should_increment) {
        ...
        if (changes && changes.length > 0) {
            const isConflictingInfo = is_conflicting(changes, platform_config.config_munge, this, true /* always force overwrite other edit-config */);
    ...
    editchanges.forEach(editchange => {
        if (files[editchange.file]) {
            ...
            if (!target || target.length === 0) {
                const file_xml = self.config_keeper.get(self.project_dir, self.platform, editchange.file).data;
